...
import com.example.contacts.screens.addContact.AddContactViewModel

@Composable
fun DetailScreen(modifier: Modifier = Modifier, navController: NavController, contactId: Int) {
    Log.d("id", "DetailScreen:$contactId ")


    val vm: DetailScreenViewModel = hiltViewModel()
    vm.getContactById(contactId)
    val contact = vm.contact.collectAsState()
    Log.d("TAG", "DetailScreen:${contact.value} ")
    ...
    val number = remember { mutableStateOf(contact.value.number) }
    val address = remember { mutableStateOf(contact.value.address) }
    val nameError = remember { mutableStateOf("") }
    val numberError = remember { mutableStateOf("") }

    LaunchedEffect(key1 = contact.value) {
        contact.value.let {
            name.value = it.name
    ...
    Log.d("name", "DetailScreen:${name.value} ")
...
private fun TopAppBar(modifier: Modifier = Modifier, onClick: () -> Unit, onDone: () -> Unit) {
    androidx.compose.material3.TopAppBar(navigationIcon = {
        ...
        Text(
            text = "Edit Contact", style = TextStyle(
                fontWeight = FontWeight.W500,
                fontSize = 23.sp,
                color = Color.Black
            )
